...
**⚠️ Cette parade ne supprime pas l'enveloppe, mesuré le 03/09/2026.** Brouillon créé avec de vraies ancres, relu par le connecteur : les `href` sont **quand même** réécrits en `google.com/url?q=…&ust=…`. Seul le **texte affiché** reste propre. Le paramètre `ust` est une **date d'expiration à environ 24 h** : passé ce délai, le destinataire reçoit un avertissement Google au lieu de la page. Autrement dit, **tout lien envoyé depuis ce Gmail meurt en un jour**, ancre ou pas, texte brut ou pas.
**La seule parade qui tienne** : écrire l'adresse **avec son `https://`**, et **dire au destinataire de la recopier dans son navigateur plutôt que de cliquer**. Recopiée depuis le texte affiché, elle est exacte et ne périme jamais. Pour un lien qui engage (signature, paiement), l'écrire aussi dans un document joint, que la messagerie ne réécrit pas.
...
**Complément du 03/09/2026, même dossier, quatrième incident** : la parade « coller l'adresse en texte brut pour éviter la réécriture » a produit son propre défaut. Le 26/08, l'adresse a été écrite `demo.agenceattractor.com/elevia/app/` **sans le préfixe**. Relevé dans le message réellement envoyé : Gmail l'a linkifiée **et** enveloppée, en choisissant `http://` faute de schéma explicite, soit `google.com/url?q=http://demo.agenceattractor.com/…`. La Cliente a donc reçu son Club sur une adresse non sécurisée et l'a signalé comme point technique prioritaire. **Le texte brut ne protège de rien : c'est Gmail qui transforme, pas le logiciel du destinataire.** Une adresse s'écrit toujours avec son `https://`, ne serait-ce que pour que la réécriture parte du bon schéma.
