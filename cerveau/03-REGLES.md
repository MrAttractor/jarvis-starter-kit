...
**Extension du 25/09/2026, GetWinWorld puis tous les sites** : une purge planifiée qui efface des **lignes** en SQL laisse toutes leurs photos derrière elle. Sur GetWinWorld, la suppression automatique des articles au bout de 24h avait laissé **68 photos** en deux mois. Et le SQL ne peut pas rattraper : le déclencheur `storage.protect_delete` interdit d'effacer un objet par `delete from storage.objects`, **seule l'API Storage efface le fichier**. Toute purge passe donc par une edge function qui supprime les fichiers **avant** les lignes. Le balayage des orphelins existe désormais pour tous les sites : l'edge function `nettoyage-photos`, chaque lundi à 3h UTC (migration `0067`). **Tout nouveau site client qui stocke des photos s'inscrit dans `nettoyage_photos_config`** (bucket et préfixe de ses tables), sinon il n'est pas couvert. Garde-fous : rien n'est effacé avant 7 jours, et un bucket dont plus de la moitié paraît orpheline est bloqué et signalé, parce que c'est alors la détection qui se trompe, pas le stockage. Pour contrôler sans rien effacer, appeler la fonction avec `{"simulation": true}`.
...
**Extension du 25/09/2026, GetWinWorld** : lancer le déploiement **depuis le dossier du site lui-même** (`cd` dedans, puis `pages deploy .`). Wrangler embarque le dossier `functions/` **du répertoire courant** : lancé depuis `demo-site/` avec `public/getwinworld` en argument, il a publié sur getwinworld.net les fonctions de Beynaud. La sortie le dit, il faut la lire : « Uploading Functions bundle » sur un site qui n'a pas de fonctions est un signal d'arrêt.
