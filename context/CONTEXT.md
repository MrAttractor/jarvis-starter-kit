...
- **Partenariat XPaye Africa (EN COURS, 13/06/2026)** : XPaye (gateway paiement CI déjà intégré en prod dans Assists, merchant ID PP-F422) comme canal d'acquisition et terrain de test pour Attractor Assists. Objectif : utiliser le réseau de merchants XPaye Africa pour tester l'app sur un échantillon réel, valider le modèle économique et construire une base d'utilisateurs CI. Approche : demande de partenariat formelle en cours de préparation — MVP défini + projection financière réaliste. Lien avec Rukayatou Saka (elle voulait un agent commercial pour booster XPaye + activer son réseau CI). Document de proposition : `livrables/commercial/partenariat-xpaye-africa/proposition-partenariat.md`.
- Construction d'un **plan clair pour ma campagne de création de contenu**.
- **Latiss (Serge Beynaud)** (artiste afrobeat CI, 2M+ fans sur les réseaux) : prospect Famille A identifié. Concept = plateforme fan club propriétaire (Fan Club payant 3 niveaux, boutique, contenu exclusif, dashboard artiste temps réel) — marque concept non encore validée par l'artiste, non nommée dans les documents. Démo 11 slides sur `demo.agenceattractor.com/beynaud` avec NDA allégé en entrée (simple acceptation confidentialité RGPD, NDA complet formalisé au cadrage). `offre.html` refondée : 3 modèles en euros (Clé en main 5 000€ setup + 1 500€/mois + 12% | WIN WIN 1 500€ forfait + 0 MRR + 70% Latiss / 30% Agence | Licence 35 000€ setup + 1 500€/mois + 0%). ROI recalculé x10 / x28 / x1,7. XPaye 16 pays (partenaire historique Agence Attractor) intégré aux questions juridiques. **Statut : offre + NDA prêts, prochaine étape = cahier des charges + démarrage du chantier.**
...
- **demo.agenceattractor.com** (ACTIF) : sous-domaine Netlify configuré. Structure `livrables/clients/demo-site/public/[client]/index.html`. Déploiement auto via Netlify sur push master. Chaque nouveau client = dossier + commit + lien prêt en 2 min. **Distinct de l'app Assists** : `www.assists.agenceattractor.com` = back-office React (inscription, dashboard, catalogue, commandes) / `demo.agenceattractor.com` = mini-sites clients statiques (/creal, /template-1b, /template-1c…).
