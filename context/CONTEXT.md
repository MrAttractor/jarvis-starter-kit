...
- **J'envoie Express** : MVP d'app métier pour un client accompagné depuis 4 ans (service d'envoi de colis Abidjan-Paris-Abidjan). Deal 230€, acompte 130€ le 3 juin 2026.
- Développement en parallèle de l'**écosystème App Attractor**.
- **Refonte Attractor Assists** : onboarding en 3 actes codé et testé (30/05/2026). Acte 1 = 3 slides machine à écrire. Acte 2 = ouverture + naming + profil dominant + anamnèse 5 questions. Acte 3 = livrable Edge Function `generate-livrable` (Supabase). App sur localhost:5173. Navigation validée (30/05/2026) : Accueil / FAB (conversation directe) / Assistants / Profil. MasterSheet retirée du nav, repositionnée dans Profil sous "Ce que [assistant] sait sur toi". ActivationScreen (remplace Discovery) : démarre directement par le mirroring de l'`ouverture` capturée en onboarding, 5 temps (mirroring → confirmation → insight couloir → action WOW → victoire rapide). Analyse présence en ligne réelle via WebFetch, faite 1 seule fois, stockée dans `profiles.presence_analyse` (Ventes/Visibilité uniquement). Profil Organisation = vide-tête + diagnostic 3 fuites, pas d'analyse présence. Agents verrouillés = mode passif interactif (génie d'Aladin) : se présentent, répondent aux questions, réponse tronquée avec gradient fade + trigger upgrade. Coach = gratuit sans limite, RAG ATTRACTOR uniquement (hors scope = pas de prise en charge), réponses riches pour créer l'addiction. Migration anciens utilisateurs Apps Script → Supabase en cours. Design : miniatures illustrées expressives par agent (style Ugo Creative CI), assets à créer sur Canva Pro, emojis standards supprimés. Détail : `decisions-produit.md`, `methode-attractor-synthese.md`.
