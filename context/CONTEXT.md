...
- **Attractor Assists** : app déployée et fonctionnelle sur `assists.agenceattractor.com`. Repo `jarvis-starter-kit` sur GitHub, Netlify connecté. 25 testeurs actifs. Stack : React + Vite + Tailwind + Supabase (projet `lgdgbrivnhgeupqhkckd`, Paris). Login OTP 6 chiffres, PWA installable (logo Attractor). **7 agents** : Bras Droit (gratuit, proactif), Awa (Prospection, gratuite, proactive), Miriam (Présence digitale, Manager), Serge (Organisation, Manager), Roland (Finance, Manager), Kofi (Storytelling & Campagnes, Manager), **Carelle (Chief of Staff / Coordination, Manager — bras droit de Mac Arthur depuis 2009, délègue la vente à Awa, orchestre les agents)**. Mémoire long terme (table `memories`) + mémoire courte auto (`memoire_cache`). **MIROIR actif** : chaque session charge les principes haute confiance de `referentiel_actif` et les injecte dans tous les agents (live). 3 crons actifs : `miroir-reveil` (*/30 min), `notify-auto-matin` (8h), `collect-insights-matin` (7h). **Cockpit admin** (4 onglets) : Users, Feedback, Miroir (référentiel + décisions), Awa (CRM prospects de Mac Arthur + séquences générées). A faire : Parcours Ventes, Parcours #1dansmoncouloir, intégration paiements XPAYE, module voix. Détail : `decisions-produit.md`, `methode-attractor-synthese.md`.
...
- **CRM Prospects + Journal agents** (LIVRÉ, 02/06/2026) : table `prospects` + `sequences_vente` + `journal_agent`. Edge Function `generate-sequence` (Awa génère 3 messages calibrés). Edge Function `collect-insights` (conversations 24h → thèmes → décision MIROIR, cron 7h). CRM accessible dans cockpit admin onglet Awa uniquement.
- **Prochains chantiers identifiés** : benchmark Paperclip, schéma visuel des connexions agents (utile démos B2B), Parcours Ventes, module voix ConversationScreen, paiements XPAYE.
...
- **Règle architecturale fondamentale — compte Mac Arthur = source (validé 02/06/2026)** : Mac Arthur est le seul décideur de ce qui est injecté chez les autres utilisateurs. Circuit : Mac Arthur valide/rejette une décision dans son cockpit admin → MIROIR extrait le principe → injection dans tous les agents. Rien ne passe sans sa validation explicite. Le cockpit admin (Users, Feedback, Miroir, Awa) est son espace de pilotage personnel — les utilisateurs n'y ont pas accès. La table `prospects` est son CRM privé, pas une feature utilisateurs.
- **Frontières agents dans Attractor Assists (validé 02/06/2026)** : chaque agent a un territoire exclusif. Carelle (Chief of Staff) = coordination uniquement, jamais de vente. Toute demande de prospection/message commercial qu'elle reçoit → délégation immédiate à Awa. Awa = seule à produire des messages de vente. Pas de recouvrement entre agents.
