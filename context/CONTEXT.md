...
- **Attractor Assists** : app déployée sur `assists.agenceattractor.com`. Repo `jarvis-starter-kit` sur GitHub, Netlify auto-deploy. 25 testeurs actifs. Stack : React + Vite + Tailwind + Supabase (projet `lgdgbrivnhgeupqhkckd`, Paris). Login OTP 6 chiffres, PWA installable. **7 agents** : Coach/Bras Droit (gratuit), Awa (Prospection), Miriam (Contenu), Serge (Organisation), Roland (Finance), Kofi (Campagnes), Carelle (Chief of Staff). Mémoire long terme + MIROIR actif (crons 30min/7h/8h). MacCockpitScreen admin (4 onglets : Carelle, Agence, Pipeline, Hub). **REFONTE STRATÉGIQUE EN COURS (03/06/2026)** : décision de transformer l'app en plateforme Chief of Staff numérique pour entrepreneurs africains. Nouvelle architecture 4 tiers : Gratuit / Attractor Growth (9 900 FCFA CI | €15 EU) / Attractor Team (39€ EU | ~25 500 FCFA CI) / Application Personnalisée (barème Famille A). Plan d'exécution en 4 phases. **Phase 1 LIVRÉE (03/06/2026)** : OnboardingScreen réécrit (narrative Mr Attractor, 3 questions CoS, quick win carnet), CarnetAffairesScreen (CRM léger clients+prospects, deeplink WA, alerte inactivité), DashboardScreen restructuré (quick actions, conseil clients inactifs), InstallScreen avec icône app visible. Migrations SQL 0017 (carnet_affaires) + 0018 (plan_codes : decouverte→gratuit, manager→team) appliquées. **Phase 2 LIVRÉE (03/06/2026)** : DechargeVocaleScreen (micro → Whisper transcrit → Claude Haiku extrait tâches/clients/rappels/idées → sauvegarde sélective), Edge Function `process-dump` déployée (OPENAI_API_KEY configurée). **Phase 3 LIVRÉE (03/06/2026)** : PaliersScreen redesignée 4 tiers avec hero charbon + barre de progression visuelle, MOCK.forfaits restructuré (features avec titre+détail dépliables), XPAYE stub dans PaymentSheet. AssistantsScreen : hero charbon avec avatars circulaires cliquables + carousel horizontal 6 agents (snap CSS, dots, swipe natif, slides portrait plein écran). DashboardScreen : hero immersif 260px avec image de fond + dégradé orange→charbon, salutation contextuelle heure/jour, tagline "Qu'est-ce qu'on règle aujourd'hui ?". Onglet "Mon équipe" : icône `users` (remplace `bot`). Fix encodage : 6 fichiers screens corrigés (garbled UTF-8 via Windows-1252). **Phase 4 LIVRÉE (03/06/2026)** : accès agents dynamiques (agentGating.js), démo Carelle accessible depuis plan Gratuit (conversation 4 questions + maquette auto + tracking prospect), Marketplace onglet "Experts" (prestataires vérifiés, formulaire inscription, validation Hub cockpit). Migration SQL 0019 appliquée. chat-assistant redéployé avec mode démo Carelle.
...
- **Prochains chantiers identifiés** : Awa qualification flow (questions avant séquence), Parcours Ventes, paiements XPAYE (sandbox PP-F422 validé), Parcours #1dansmoncouloir, Carelle → Programmeur → Maquette automation.
...
- **Prochaines étapes (pas encore actifs)** : Stripe, CinetPay.
