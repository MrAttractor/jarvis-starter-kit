...
- **Attractor Assists** : app déployée et fonctionnelle sur `assists.agenceattractor.com`. Repo `jarvis-starter-kit` sur GitHub, Netlify connecté. 25 testeurs actifs. Stack : React + Vite + Tailwind + Supabase (projet `lgdgbrivnhgeupqhkckd`, Paris). Login OTP 6 chiffres, PWA installable (logo Attractor). **7 agents** : Bras Droit (gratuit, proactif), Awa (Prospection, gratuite, proactive), Miriam (Présence digitale, Manager), Serge (Organisation, Manager), Roland (Finance, Manager), Kofi (Storytelling & Campagnes, Manager), **Carelle (Chief of Staff / Coordination, Manager)**. Mémoire long terme (table `memories`) + mémoire courte auto (`memoire_cache`). **MIROIR actif** : cron réveil 30 min + cron insights 7h + cron notifs 8h. **MacCockpitScreen** (LIVRÉ, 02/06/2026) : tableau de bord sur mesure pour le compte admin de Mac Arthur — 4 onglets : Carelle (hub de commande dark, voix/TTS, contexte agence injecté, une question à la fois), Agence (31 agents Jarvis + app avec rôle/mécanisme/historique, section entraînement Awa), Pipeline (prospects + maquette-closer + devis + suppression/statut), Hub (stats users + tendances utilisation agents + broadcasts + MIROIR + feedbacks + déconnexion). Bypass onboarding/activation pour admin, connexion directe sur Carelle. **Mon équipe vivant** (02/06/2026) : badges Supabase live, teasers temps-réels, strip activité, quick actions contextuelles. **Maquette closer flow** (Famille A) : upload image/URL référence, message Awa pré-rédigé, brief auto Carelle au copy. Fix encodage complet (ConversationScreen garbled chars). SQL 0016 EXÉCUTÉ (compte macarthur.nguessankouassi@gmail.com → manager + admin). À faire : Parcours Ventes, Parcours #1dansmoncouloir, intégration paiements XPAYE, module voix ConversationScreen, Awa qualification flow. Détail : `decisions-produit.md`, `methode-attractor-synthese.md`.
