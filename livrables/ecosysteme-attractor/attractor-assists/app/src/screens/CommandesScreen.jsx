...
function PalierBanner({ count, go }) {
  ...
  return (
    <div className={`mx-4 mb-3 rounded-2xl px-4 py-3 flex items-center justify-between gap-3 ${blocked ? 'bg-red-50 border border-red-200' : 'bg-orange/8 border border-orange/20'}`}>
      ...
      <button
        onClick={() => go('paliers')}
        className="flex-shrink-0 h-11 px-4 rounded-xl bg-orange text-white text-[12px] font-bold active:opacity-80 transition"
      >
        Growth
      </button>
    </div>
        ...
        <div className="flex bg-g100 rounded-xl p-1 gap-1 mb-3">
          {PERIODS.map(p => (
            <button
              ...
              className={`flex-1 min-h-[40px] rounded-lg text-[12.5px] font-bold transition ${
          ...
          {TABS.map(t => {
            ...
            return (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`flex-1 min-h-[40px] rounded-lg text-[12px] font-bold transition ${
          ...
          <div className="flex gap-2 flex-wrap">
            {o.client_wa && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-1.5 h-11 px-3.5 rounded-xl border border-vert/30 text-vert text-[13px] font-bold"
              >
            ...
            {o.status === 'new' && (
              <button
                ...
                className="flex-1 h-11 rounded-xl bg-orange text-white text-[13px] font-bold shadow-[0_4px_12px_-4px_rgba(242,92,5,.5)]"
              >
                Valider
              </button>
            )}
            {o.status === 'preparing' && (
              <button
                ...
                className="flex-1 h-11 rounded-xl bg-vert text-white text-[13px] font-bold"
              >
                Marquer livrée
              </button>
            )}
            {(o.status === 'new' || o.status === 'preparing') && (
              <button
                ...
                className="h-11 px-3.5 rounded-xl border border-g200 text-g400 text-[13px] font-bold"
              >
                Annuler
              </button>
            )}
