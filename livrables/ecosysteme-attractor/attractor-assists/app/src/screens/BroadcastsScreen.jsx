...
export function BroadcastsScreen({ go, notify }) {
  ...
  const locked = true;
  const planned = [
    { t: "Promo bissap âˆ’20 %", aud: "Tous Â· 312 contacts", when: "Demain Â· 11:00", on: "WhatsApp" },
    { t: "Nouveau parfum gingembre", aud: "Clients fidÃ¨les Â· 84", when: "Sam Â· 09:00", on: "Instagram" },
  ];

  return (
    <div className="min-h-screen bg-sable">
      <AppHeader title="Broadcasts" sub="Parle Ã  toute ta communautÃ© d'un coup" onBack={() => go("dashboard")} />
      <div className="px-[18px] pb-4 flex flex-col gap-4">
        {locked && (
          <button onClick={() => go("paliers")} className="rounded-[20px] p-4 text-white text-left flex items-center gap-3" style={{ background: "linear-gradient(120deg,#1F1B18,#2a2320)" }}>
            ...
            <div className="flex-1">
              ...
              <p className="text-[12.5px] text-white/70">Compose ici en dÃ©mo Â· passe Manager pour envoyer.</p>
            </div>
        ...
        <Card className="p-5 flex flex-col gap-4">
          <Field label="Ton message">
            <Textarea rows={4} placeholder="Salut la famille ! Cette semaine chez Ayaâ€¦" value={msg} onChange={e => setMsg(e.target.value)} />
            <div className="flex justify-between mt-1.5">
              <span className="text-[11.5px] text-g400">{msg.length} caractÃ¨res</span>
              <button onClick={() => go("axes")} className="text-[12px] font-bold text-orange flex items-center gap-1"><Icon name="spark" size={13} /> Aide-moi Ã  Ã©crire</button>
            </div>
          </Field>
          <div>
            <span className="block font-semibold text-[13.5px] mb-2">Audience</span>
            <div className="flex flex-wrap gap-2">
              {["Tous mes contacts", "Clients fidÃ¨les", "Prospects"].map(x => (
          ...
          <Btn className="w-full" icon={locked ? "lock" : "send"} onClick={() => locked ? go("paliers") : notify("Broadcast envoyÃ© âœ“")}>
            {locked ? "DÃ©bloquer l'envoi (Manager)" : "Envoyer le broadcast"}
          </Btn>
        </Card>

        <SectionLabel>PlanifiÃ©s (aperÃ§u)</SectionLabel>
        {planned.map((b, i) => (
          <Card key={i} className="p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12px] text-g400">{b.aud} Â· {b.on}</p>
            </div>
