...
export function AxesScreen({ go, notify }) {
  ...
  return (
    <div className="min-h-screen bg-sable">
      <AppHeader title="Axes de communication" sub="Tes argumentaires prÃªts Ã  copier" onBack={() => go("dashboard")} />
      <div className="px-[18px] pb-4 flex flex-col gap-4">
        <Card className="p-5 flex flex-col gap-5">
          <div>
            <span className="block font-semibold text-[13.5px] mb-2">MÃ©thode</span>
            <div className="grid grid-cols-2 gap-2.5">
              {[["AIDA", "Attention Â· IntÃ©rÃªt Â· DÃ©sir Â· Action"], ["PASA", "ProblÃ¨me Â· Agitation Â· Souhait Â· Action"]].map(([k, d]) => (
          ...
          <div>
            <span className="block font-semibold text-[13.5px] mb-2">RÃ©seau</span>
          ...
          <Field label="Angle (optionnel)" hint="Laisse vide pour partir de ta MasterSheet.">
            <Input placeholder="Ex : promo de lancement, fraÃ®cheur, livraison rapideâ€¦" value={angle} onChange={e => setAngle(e.target.value)} />
          </Field>
          <Btn className="w-full" icon="spark" onClick={generate}>GÃ©nÃ©rer mon argumentaire</Btn>
        </Card>

        {state === "loading" && (
          <Card className="p-8 flex flex-col items-center gap-3 text-center">
            <Spinner className="w-10 h-10" />
            <p className="font-display font-bold text-[15px] text-charbon">{MOCK.user.assistantName} rÃ©dige ton {framework}â€¦</p>
        ...
        {state === "error" && (
          <Card className="p-6 flex flex-col items-center gap-3 text-center">
            ...
            <p className="font-display font-bold text-[15px]">Oups, Ã§a a coincÃ©.</p>
            <p className="text-[13px] text-g400 max-w-[260px]">La gÃ©nÃ©ration n'a pas abouti. RÃ©essaie, ton crÃ©dit n'a pas Ã©tÃ© touchÃ©.</p>
            <Btn variant="ghost" icon="refresh" onClick={generate}>RÃ©essayer</Btn>
          </Card>
        )}

        {state === "result" && (
          <div className="animate-[fadeUp_.3s_ease]">
            <SectionLabel className="mb-3" action={<Pill tone="info">{channel}</Pill>}>RÃ©sultat â€” {framework}</SectionLabel>
            <Card className="overflow-hidden">
              ...
              <div className="px-4 pb-4 flex gap-2">
                <Btn className="flex-1 !py-3 !text-[14px]" icon="copy" onClick={() => notify("CopiÃ© âœ“ â€” prÃªt Ã  coller")}>Copier</Btn>
                <button onClick={() => notify("Variante gÃ©nÃ©rÃ©e")} className="w-12 rounded-xl border-[1.5px] border-g200 flex items-center justify-center text-g700 hover:border-orange/50"><Icon name="refresh" size={19} /></button>
