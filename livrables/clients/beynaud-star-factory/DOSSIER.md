...
| D-19 | **`latiss.net` et `www.latiss.net` sont deux origines distinctes**, sans redirection canonique, et le HTTP n'est pas forcé en HTTPS | `localStorage` étant propre à chaque origine, un fan inscrit sur l'une **perd son compte** en revenant sur l'autre : ni prénom, ni grade, ni lien de parrainage | ouvert, **à régler avant la bascule de l'espace fan** |
