...
| D-16 | **L'encaissement n'est pas branché.** Un billet ne s'écrit aujourd'hui qu'à la main ou par la fonction serveur : rien ne relie XPaye à `bey_donner_billet` | **on peut réserver un contenu, pas encore le vendre** | ouvert, **bloquant pour le 5 décembre** |
| D-17 | **L'espace de Serge ne sait pas cocher « payant ».** `launchLive` écrit `grade_requis:'membre'` en dur et aucun formulaire n'expose `billet_requis` | le verrou existe mais Serge ne peut pas l'activer seul | ouvert |
