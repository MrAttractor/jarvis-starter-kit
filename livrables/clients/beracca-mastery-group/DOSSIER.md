# Beracca Mastery Group — l'état du dossier

> Révision du 31/07/2026. **Cette fiche est la première chose à lire du dossier.**

## En une phrase

Documents envoyés le 17/06. **En attente de la cliente et du versement de lancement.
Aucune relance tracée depuis six semaines.**

## Le client

**Bérénice KOUADIO DEBRIMOU**, beraccamasterygroup@gmail.com, +225 07 57 49 72 07.
Agro-distribution, marque **TOP Attiéké** (100 % manioc local, made in Côte d'Ivoire).

**Le fait qui donne sa valeur au dossier** : Beracca a obtenu, via la mise en relation
faite par Mac Arthur avec le Cabinet DAB, un **marché d'exportation de 3 à 5 tonnes par
mois d'attiéké vers KONAVA / magasins OPLUS au Canada**. C'est le premier producteur à
avoir matérialisé le concept DFMM. Voir `../cabinet-dab/`.

## Le modèle de deal, à connaître

C'est la **Famille D, partenariat de performance**, le seul modèle de ce type de l'agence :

| | |
|---|---|
| Mise en place | **150 € / 100 000 FCFA** |
| Mensuel | **150 €/mois** |
| Intéressement | **10 % du bénéfice réalisé au-dessus de la moyenne des 3 derniers trimestres** |

**L'intéressement porte sur le bénéfice, pas sur le chiffre d'affaires**, et seulement sur
le delta par rapport à une moyenne mobile. C'est ce qui rend le modèle défendable : on ne
gagne que si l'entreprise gagne plus qu'avant.

**Encaissé : rien.** Le lancement est conditionné au versement des 100 000 FCFA.

## Ce qui fait foi

| Document | Fichier |
|---|---|
| Contrat, 15 articles, 5 phases ERP | `CONTRAT-Beracca-MrAttractor-2026-06-17.html` |
| NDA, 11 articles, arbitrage CCJA-OHADA | `NDA-Beracca-MrAttractor-2026-06-17.html` |
| Fiche d'investissement synthétique | `FICHE-INVESTISSEMENT-Beracca-2026-06-17.html` |
| Proposition | `proposition-bmg.html` / `.pdf` |
| Synthèse stratégique | `synthese-strategique.md` |

## Prochaine action

**Relancer.** Le dossier dort depuis le 17/06 et il a deux atouts rares : un marché
export déjà obtenu, et un modèle où l'agence est payée à la performance. Si elle ne donne
pas suite, le savoir explicitement et fermer, plutôt que de laisser le dossier ouvert
indéfiniment.
